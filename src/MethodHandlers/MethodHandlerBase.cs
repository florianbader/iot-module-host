...
public abstract class MethodHandlerBase : IMethodHandler
{
    ...
    /// <inheritdoc />
    async Task<MethodResponse> IMethodHandler.HandleMethodAsync(MethodRequest methodRequest, CancellationToken cancellationToken)
    {
        try
        {
            Logger.LogTrace("Executing method handler {MethodHandlerName}", _name);

            var response = await HandleMethodAsync(methodRequest, cancellationToken).ConfigureAwait(false);
    ...
    protected abstract Task<MethodResponse> HandleMethodAsync(MethodRequest methodRequest, CancellationToken cancellationToken);
